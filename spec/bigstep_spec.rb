...
RSpec.describe 'BigStep' do
  describe '#evaluate' do
    let(:environment) { {} }

    context 'Expression' do
      subject { expression.evaluate(environment) }
      let(:environment) { {} }
    ...
    context 'Statement' do
      subject { statement.evaluate(environment) }
      let(:environment) { {} }
      ...
      context 'Assign(x = x + 1)' do
        ...
        let(:environment) { { x: Number.new(1) } }
        it { is_expected.to eq x: Number.new(2) }
      end
      ...
      context 'of While(x < 10) { x = x + 1 }' do
        ...
        it { is_expected.to eq x: Number.new(10) }
      end
    end
  end
end
